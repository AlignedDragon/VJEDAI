"""Video-Depth-Anything original loss recipe, vendored verbatim (Apache-2.0).

This is the LITERAL VDA training loss: TrimmedProcrustesLoss (affine-invariant,
linear-depth) + TemporalGradientMatchingLoss, operating on (B, T, H, W). It is
kept here for provenance and as an opt-in alternative. The default VJEDAI video
training uses src/video_depth_loss.py::vjedai_video_depth_loss instead, which stays in
the log-space, per-image scale-invariant world that matches the leaderboard
siRMSE metric and the Gaussian-NLL uncertainty head. Use this module only if you
deliberately want VDA's affine-invariant objective.
"""
